...
def department(sentence):
    # with graph.as_default():
    # sentence = input('please input sentence:\n')
    # sentence = request.args.get("question")
    if sentence == None:
        return 'sentence must be  not None'
    # testcase = ['基金发力持续营销 主打定投牌□本报记者 徐国杰 上海报道 随着前期股市反弹', '梨树种下之后，过几年能结果子？', '青蛙在吃虫子时眼睛是怎样的状态？']
    # aaa = time.clock()
    # for t in testcase:
    # print()

    return str(toy.predict_on_ckpt(sentence))
