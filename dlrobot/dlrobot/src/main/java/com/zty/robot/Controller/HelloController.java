...
public class HelloController {
    ...
    /*    if(message.contains("你")&&message.contains("谁")){
    ...
    public String faqQuestion(String ques){
        ...
        parm.append("question=").append(ques);
        String result = HttpUtil.sendPost(AI_URL,parm.toString());
     /*   String ans=null;
       List<Faq> faqlist=  faqService.findAll();
       for(Faq faq:faqlist){
         faq.setIndexNum(contrastStr(ques,faq.getQuestion()));

       }
        double index=0;
        int maxIndex=0;
        index=faqlist.get(0).getIndexNum();
        for(int i=0;i<faqlist.size();i++){
            if( faqlist.get(i).getIndexNum()>index){
                index=faqlist.get(i).getIndexNum();
                maxIndex=i;
            }
        }
           ans= faqlist.get(maxIndex).getAnswers();
        if(maxIndex==0&&faqlist.get(0).getIndexNum()==0){
            ans=null;
        }*/
        return result;
    }
